...
The small original raster art pack in `assets/` adds the animated lantern keeper and discovery sprites while retaining the game's no-dependency static setup. Luna's current four-frame v7 walk atlas is a direct, transparency-only 26 × 40-cell reduction of the exact retained owner attachment; runtime drawing scales those unchanged cells to 13 × 20 with nearest-neighbor rendering. Eir's unchanged runtime strip now draws at 16 × 24—exactly half her former size—with nearest-neighbor rendering, the same grounded baseline, and her existing non-solid 22-pixel interaction reach. Luna's 10 × 10 movement box is unchanged. The current forest system is derived from the owner-selected framed bottom-right clearing: a singular crescent landmark, broad mossy root shelves, continuous loam, cool moonlight pools, enclosing spruce canopies, raster Eir, warm fireflies, and supporting foliage are integrated as separate production layers across all four levels. Its provenance and reuse note are recorded in `assets/ARTWORK.md`.
