...
The small original raster art pack in `assets/` adds the animated lantern keeper and discovery sprites while retaining the game's no-dependency static setup. Luna's current four-frame v7 walk atlas is a direct, transparency-only 26 × 40-cell reduction of the exact retained owner attachment, and Eir's runtime strip and portrait use corrected no-violet outlines. Luna's visual overhang is larger while her 10 × 10 movement box is unchanged. The current forest system is derived from the owner-selected framed bottom-right clearing: a singular crescent landmark, broad mossy root shelves, continuous loam, cool moonlight pools, enclosing spruce canopies, raster Eir, warm fireflies, and supporting foliage are integrated as separate production layers across all four levels. Its provenance and reuse note are recorded in `assets/ARTWORK.md`.
