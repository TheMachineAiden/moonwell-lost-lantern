...
The small original raster art pack in `assets/` adds the animated lantern keeper and discovery sprites while retaining the game's no-dependency static setup. Luna's current four-frame v8 walk atlas preserves every authored pixel of the direct, transparency-only v7 reduction from the exact retained owner attachment and adds only a muted one-source-pixel teal contact edge. Runtime drawing keeps the unchanged 13 × 20 nearest-neighbor footprint, anchor, gait, and 10 × 10 movement box. Her Whispering Hollow replay remains a retained cool-cyan derivative of the same exact-owner v7 poses, with the unchanged draw box, anchor, path, and timing. Eir's runtime strip draws at 16 × 24 with nearest-neighbor rendering, the same grounded baseline, and her existing non-solid 22-pixel interaction reach. The current forest system is derived from the owner-selected framed bottom-right clearing: parted-spruce route openings, broad mossy root shelves, continuous loam, cool moonlight pools, enclosing spruce canopies, raster Eir, warm fireflies, and supporting foliage are integrated as separate production layers across all four levels. Its provenance and reuse note are recorded in `assets/ARTWORK.md`.
