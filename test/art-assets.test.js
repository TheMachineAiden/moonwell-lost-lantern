...
test('Moonroot bridge keeps its retained footprint while yielding warm hierarchy to the crossing',()=>{
  ...
  assert.match(html,/script src="game\.js\?v=moonwell-hollow-echo-1"/);
...
test('Starfall uses grounded cool-dormant starroot art and contains no sky-bell runtime path or copy',()=>{
  ...
  assert.match(html,/game\.js\?v=moonwell-hollow-echo-1/);
});
...
test('Eir dialogue uses raster production art and has no SVG or drawn-sigil fallback',()=>{
  ...
  assert.equal(createHash('sha256').update(read('assets/moonwell-art/production/moonwell-eir-rootwatcher-idle-v2.png')).digest('hex'),'4bc70c41fd73e083af2f1654ee38273981da949928c57229b553831f6d22a4cd');
  assert.doesNotMatch(source+html,/moonwell-eir-rootwatcher-idle-v3|process-eir-rootwatcher-readability-art/);
